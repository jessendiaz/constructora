...
$varDato_rutaImagen = "0";
if (isset($_GET["recordID"])) {
  $varDato_rutaImagen = $_GET["recordID"];
}
mysql_select_db($database_conexionconstructora, $conexionconstructora);
$query_rutaImagen = sprintf("SELECT tblslider.strImagen FROM tblslider WHERE tblslider.idContador=%s", GetSQLValueString($varDato_rutaImagen, "int"));
$rutaImagen = mysql_query($query_rutaImagen, $conexionconstructora) or die(mysql_error());
$row_rutaImagen = mysql_fetch_assoc($rutaImagen);
$totalRows_rutaImagen = mysql_num_rows($rutaImagen);
...
if ((isset($_GET['recordID'])) && ($_GET['recordID'] != "")) {
  ...
  $deleteGoTo = "sliderlista.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
    $deleteGoTo .= $_SERVER['QUERY_STRING'];
  }
  header(sprintf("Location: %s", $deleteGoTo));
  $a="../documentos/img_slider/";
  $b=$a.$row_rutaImagen['strImagen'];
  unlink($b);/* eliminar imagen de archivo */
}
?>
<?
...
<div class="container">
  ...
  <div class="content"><!-- InstanceBeginEditable name="partederechaadmin" -->
    <h1>Eliminando Imagen Slider</h1>
    <p>Procesando...</p>
  <!-- InstanceEndEditable -->
